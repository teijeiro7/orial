...
export class WeightPredictionService {
  ...
  private readonly GLYCOGEN_WATER_RATIO = 3.5; // grams of water per gram of glycogen
  private readonly SODIUM_WATER_RETENTION = 0.0015; // kg per mg of sodium
  private readonly FIBER_TRANSIT_WEIGHT = 0.5; // kg per 10g fiber

  static getInstance(): WeightPredictionService {
    if (!WeightPredictionService.instance) {
      WeightPredictionService.instance = new WeightPredictionService();
    }
    return WeightPredictionService.instance;
  }

  async generatePrediction(date: string): Promise<WeightPrediction | null> {
    ...
    // 2. Sodium retention
    if (m?.sodiumMg) {
      const sodiumRetention = m.sodiumMg * this.SODIUM_WATER_RETENTION;
    ...
    if (m?.carbsG) {
      ...
      const waterChange = glycogenChange * this.GLYCOGEN_WATER_RATIO;
    ...
    // 4. Fiber/transit
    if (m?.fiberG) {
      const fiberWeight = (m.fiberG / 10) * this.FIBER_TRANSIT_WEIGHT;
    ...
    const confidence = this.calculateConfidence(m, h, w);
  ...
  async updateActualWeight(date: string, weightKg: number): Promise<void> {
  ...
  async getTodayPrediction(): Promise<WeightPrediction | null> {
    ...
    return this.generatePrediction(today);
  }
  ...
  async getAccuracy(days: number = 14): Promise<{ mae: number; withinRange: number; total: number }> {
  ...
  private calculateConfidence(manual: typeof manualMetrics.$inferSelect | undefined, hydrationData: typeof hydration.$inferSelect | undefined, whoop: typeof whoopDaily.$inferSelect | undefined): number {
...
export const weightPredictionService = WeightPredictionService.getInstance();
