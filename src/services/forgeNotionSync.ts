...
async function getNotionSettings(): Promise<{
...
async function syncWeightEntry(entry: {
  id: string;
  date: Date;
  ...
  try {
    ...
    await createNotionPage(settings.notionLogsDbId, {
      Name: {
        title: [
          {
            text: {
...
async function syncDailyMetrics(date: string): Promise<void> {
  const settings = await getNotionSettings();
